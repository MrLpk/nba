...
def count1(obj, key):
	...
	return mean, _median, _lmedian, _rmedian, _hScore, _lScore

def countOneTeam1(obj):
	_t = obj['t']
	_tw, _twm, _twlm, _twrm, _twmax, _twmin = count1(_t, 'w')
	_tl, _tlm, _tllm, _tlrm, _tlmax, _tlmin = count1(_t, 'l')

	_f = obj['f']
	_fw, _fwm, _fwlm, _fwrm, _fwmax, _fwmin = count1(_f, 'w')
	_fl, _flm, _fllm, _flrm, _flmax, _flmin = count1(_f, 'l')

	return {'t':{'w':_tw, 'wm':_twm, 'wlm':_twlm, 'wrm':_twrm, 'wmax':_twmax, 'wmin':_twmin, 
				 'l':_tl, 'lm':_tlm, 'llm':_tllm, 'lrm':_tlrm, 'lmax':_tlmax, 'lmin':_tlmin}, 
			'f':{'w':_fw, 'wm':_fwm, 'wlm':_fwlm, 'wrm':_fwrm, 'wmax':_fwmax, 'wmin':_fwmin,
				 'l':_fl, 'lm':_flm, 'llm':_fllm, 'lrm':_flrm, 'lmax':_flmax, 'lmin':_flmin}
		   }
